process.env.NODE_ENV = "test";
process.env.PROCESS_PORT = "3000";
process.env.REDIS_SERVER = "localhost";
process.env.REDIS_PORT = "6379";
process.env.REDIS_PASSWORD = "";
process.env.REDIS_SSL = "false";
process.env.REDIS_STACK = "true";
process.env.POSTGRES_SERVER = "localhost";
process.env.POSTGRES_PORT = "5432";
process.env.POSTGRES_USER = "user";
process.env.POSTGRES_PASSWORD = "password";
process.env.POSTGRES_DATABASE = "db";
process.env.RATE_LIMIT_MAX = "100";
process.env.RATE_LIMIT_WINDOW = "60";
process.env.AUTH_SALT = "salt";
process.env.SHOW_LOG = "false";
process.env.LOG_LEVEL = "info";
process.env.APP_NAME = "Test App";
process.env.APP_DESCRIPTION = "Test Description";
process.env.APP_VERSION = "0.0.1";
process.env.PROCESS_CORS_ORIGIN = "*";
process.env.PROCESS_DOMAIN = "http://localhost:3000";
process.env.APP_FOLDER_KEY = "test-folder-key";
process.env.APP_HTTP2 = "false";
process.env.APP_CERT = "./keys/cert.pem";
process.env.APP_KEY = "./keys/private.pem";
process.env.APP_TRUST_PROXY = "false";
process.env.SSO_GOOGLE_CLIENT_ID = "test-client-id";
process.env.SSO_GOOGLE_CLIENT_SECRET = "test-client-secret";
process.env.SSO_GOOGLE_REDIRECT_URI = "http://localhost/callback";
process.env.SSO_GITHUB_CLIENT_ID = "test-github-id";
process.env.SSO_GITHUB_CLIENT_SECRET = "test-github-secret";
process.env.SSO_GITHUB_REDIRECT_URI = "http://localhost/callback/github";
