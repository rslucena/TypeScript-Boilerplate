...
> \[!TIP\] **Coercion:** Zod allows you to coerce strings from `.env` into their actual types. For example, `z.coerce.number()` transforms the string "3000" into the number `3000`, and `.transform((val) => val === "true")` creates a boolean.
...
> \[!WARNING\] If you are adding third-party scripts or changing how your Swagger UI is hosted, you might need to adjust the CSP directives in `helmet.ts` to allow those specific CDNs.