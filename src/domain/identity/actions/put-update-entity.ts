...
export default async function putUpdateEntity(request: container) {
	...
	if (!validParams.success) throw request.badRequest(request.language(), tag("identity", "put{id}"));

	const validBody = await schema.actions.update.safeParseAsync(request.body());
	if (!validBody.success) throw request.badRequest(request.language(), tag("identity", "put{id}"));

	const content = await repository
		.update(identity)
		.set({ ...validBody.data, updatedAt: new Date() })
		.where(eq(identity.id, validParams.data.id))
		.returning();

	if (!content.length) throw request.notFound(request.language(), tag("identity", "put{id}"));
