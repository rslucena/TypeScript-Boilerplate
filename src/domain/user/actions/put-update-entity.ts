...
export default async function putUpdateEntity(request: container) {
	...
	if (!validParams.success) throw request.badRequest(request.language(), tag("products", "update{params}"));

	const validBody = await schema.actions.update.safeParseAsync(request.body());
	if (!validBody.success) throw request.badRequest(request.language(), tag("products", "update{body}"));
	...
	if (!content.length) throw request.notFound(request.language(), tag("products", "update"));

	await cache.json.del(tag("products", "find*"));
