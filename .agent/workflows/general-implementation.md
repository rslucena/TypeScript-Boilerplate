...
2. **Context Analysis**:
   ...
   - Check for relevant architectural decisions in `wiki/Architecture-Decisions.md`.
...
4. **Development Cycle**:
   ...
   - Use the linter: `bun biome lint src/`.
   - Run relevant tests: `bun test <path_to_test>`.
   - **Mandatory**: Ensure tests cover **90% to 100%** of the new logic.

5. **Documentation & Cleanup**:
   - **Validation**: Check if `README.md` or `wiki/` needs updates, additions, or improvements based on the change.
   ...
   - **Agenic Memory**: Create or update the task log in `.agent/tasks/` and add any relevant findings to `.agent/reflections/`.
