...
- **Final Cleanup**: After code validation and final approval, all comments (except for necessary JSDoc for API documentation) must be **removed**. Additionally, remove all `console.log` and temporary files (`.log`, `temp-*`) used for debugging or validation to keep the codebase and environment as clean as possible.
...
- **Documentation**:
    ...
    - **Graphics**: Use **VueFlow** (`<InteractiveFlow />`) components to visualize complex data flows, logic, or architectures in the documentation. Use Vue SFC script setups (`<script setup>`) internally to define nodes and edges with colors. **Mermaid** should ONLY be used in `IMPLEMENTATION_PROPOSAL.md` for PR descriptions, NEVER in the actual documentation files.
